...
export function FilterSelect({ label, param, options, fallback = "" }: { label: string; param: string; options: { value: string; label: string }[]; fallback?: string }) {
...
export function FilterToggle({ label, param, on = "1" }: { label: string; param: string; on?: string }) {
  const [value, setValue] = useSearchParamState(param, "");
  const id = `toggle-${param}`;
  return (
    <span className="inline-flex items-center gap-1">
      <input id={id} type="checkbox" checked={value === on} onChange={(e) => setValue(e.target.checked ? on : undefined)} />
